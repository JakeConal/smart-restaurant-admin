        ...
        const timer = setInterval(() => {
            setCountdown((prev) => {
                if (prev <= 1) {
                    ...
                    router.replace(`/login${currentToken ? `?token=${currentToken}` : ""}`);
                    return 0;
                }
                return prev - 1;
            });
        ...
        <div className="min-h-screen bg-ivory-100 flex items-center justify-center p-6 text-center">
            <div className="bento-card max-w-sm w-full py-12 space-y-6">
                <div className="w-20 h-20 bg-slate-100 rounded-[24px] flex items-center justify-center mx-auto shadow-inner group">
                    <svg className="w-10 h-10 text-slate-400 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-body text-slate-500">You've been securely logged out.</p>
                </div>

                <div className="pt-4 flex flex-col items-center space-y-4">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full border-2 border-slate-200 border-t-slate-700 animate-spin"></div>
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Redirecting in {countdown}s</span>
                    </div>
