...
export default function OrderTrackingPage({
  ...
  const getStatusInfo = (status: OrderStatus) => {
    const configs: Record<string, { label: string; description: string; color: string; progress: number; icon: string }> = {
      pending_acceptance: {
              ...
              <section key={displayOrder.id} className="space-y-6">
                ...
                <div className="flex items-end justify-between px-1">
                  <div>
                    ...
                    <h3 className="text-xl font-black text-slate-900">#{String(displayOrder.id).slice(-6).toUpperCase()}</h3>
                  </div>
                  <div className="text-right">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Station</span>
                    <p className="font-bold text-slate-900">Table {displayOrder.tableNumber}</p>
                  </div>
                </div>
                ...
                <div className="bento-card bg-white p-8 space-y-8 relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-6">
                    <div className={`w-16 h-16 rounded-[24px] ${statusInfo.color} text-white flex items-center justify-center text-2xl shadow-lg border border-white/20`}>
                      {statusInfo.icon}
                    </div>
                    <div className="space-y-1">
                      <h4 className="text-xl font-black text-slate-900 leading-none">{statusInfo.label}</h4>
                      <p className="text-sm text-slate-500 font-medium">{statusInfo.description}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-1">Items in this Batch</span>
                  <div className="grid grid-cols-1 gap-3">
                    {displayOrder.items.map((item) => (
                      <div key={item.id} className="bento-card bg-white/50 border-dashed p-4 flex justify-between items-center group">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-sm font-bold text-slate-900">{item.menuItemName}</p>
                ...
                {displayOrder.isPaid && (
                  <div className="bento-card bg-emerald-500 p-6 text-white flex items-center gap-4 shadow-emerald-200">
                    ...
                    <div>
                      <h5 className="text-[10px] font-black uppercase tracking-widest text-white/80">Milestone</h5>
        ...
        <section className="space-y-6 pb-12">
          <div className="bento-card bg-slate-900 p-8 text-white space-y-6 relative overflow-hidden">
            ...
            <div className="flex justify-between items-start">
              <div>
                <h5 className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400 mb-1">Account Summary</h5>
            ...
            {allOrdersReady && unpaidOrders.length > 0 ? (
              ...
              <div className="flex items-center gap-3 p-4 bg-white/5 rounded-2xl border border-white/10">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Pay after meal service enabled</p>
              </div>
            )}
          </div>
        </section>
